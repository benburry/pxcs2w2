<div class="card-box{% if longbox %} long-card-box{% endif %}{% if card.solved %} solved{% endif %}">
	{% if firstsolve %}<p class="firstsolve">First solved by <a href="{% url solved_view firstsolve.user.username%}">{{ firstsolve.user.username }}</a> on {{firstsolve.solved|date:"D jS N"}}</p>{%endif%}
